import type { Context } from "hono";
import sendResponse from "../lib/send-response.js";
import { BadRequestException } from "../lib/exception.js";
import { savingsService } from "../services/savings.service.js";
import { paymentService } from "../services/payment.service.js";
import type {
  CreateSavingsJarInput,
  DepositToSavingsJarInput,
  UpdateSavingsJarInput,
} from "../schemas/savings.schema.js";

class SavingsController {
  async list(ctx: Context) {
    const userId = ctx.get("userId") as string;
    const jars = await savingsService.list(userId);
    return sendResponse.success(ctx, "Savings jars fetched", 200, jars);
  }

  async get(ctx: Context) {
    const userId = ctx.get("userId") as string;
    const jarId = ctx.req.param("id");
    if (!jarId) throw new BadRequestException("Jar id is required");
    const data = await savingsService.getWithDeposits(userId, jarId);
    return sendResponse.success(ctx, "Savings jar fetched", 200, data);
  }

  async create(ctx: Context) {
    const userId = ctx.get("userId") as string;
    const input = ctx.get("validatedData") as CreateSavingsJarInput;
    const jar = await savingsService.createJar(userId, input);
    return sendResponse.success(ctx, "Savings jar created", 201, jar);
  }

  async update(ctx: Context) {
    const userId = ctx.get("userId") as string;
    const jarId = ctx.req.param("id");
    if (!jarId) throw new BadRequestException("Jar id is required");
    const input = ctx.get("validatedData") as UpdateSavingsJarInput;
    const jar = await savingsService.update(userId, jarId, input);
    return sendResponse.success(ctx, "Savings jar updated", 200, jar);
  }

  async remove(ctx: Context) {
    const userId = ctx.get("userId") as string;
    const jarId = ctx.req.param("id");
    if (!jarId) throw new BadRequestException("Jar id is required");
    await savingsService.remove(userId, jarId);
    return sendResponse.success(ctx, "Savings jar deleted", 200, null);
  }

  async deposit(ctx: Context) {
    const userId = ctx.get("userId") as string;
    const jarId = ctx.req.param("id");
    if (!jarId) throw new BadRequestException("Jar id is required");
    const { amount } = ctx.get("validatedData") as DepositToSavingsJarInput;
    const result = await paymentService.createSavingsFunding(userId, jarId, amount);
    return sendResponse.success(ctx, "Savings funding started", 201, {
      pendingPaymentId: result.pendingPayment.id,
      orderRefId: result.pendingPayment.orderRefId,
      virtualAccountNumber: result.virtualAccountNumber,
      accountName: result.accountName,
      bankName: result.bankName,
      amount: result.pendingPayment.amount,
      expiresAt: result.pendingPayment.expiresAt,
    });
  }

  async verifyDeposit(ctx: Context) {
    const jarId = ctx.req.param("id");
    if (!jarId) throw new BadRequestException("Jar id is required");
    const body = (await ctx.req.json().catch(() => ({}))) as { pendingPaymentId?: string };
    if (!body.pendingPaymentId) throw new BadRequestException("pendingPaymentId is required");
    const result = await paymentService.reconcileSavingsOnce(body.pendingPaymentId, jarId);
    return sendResponse.success(ctx, "Deposit status fetched", 200, {
      status: result.status,
      amount: result.amount,
    });
  }

  async cancelDeposit(ctx: Context) {
    const jarId = ctx.req.param("id");
    if (!jarId) throw new BadRequestException("Jar id is required");

    const body = (await ctx.req.json().catch(() => ({}))) as { pendingPaymentId?: string };
    if (!body.pendingPaymentId) throw new BadRequestException("pendingPaymentId is required");

    console.log(`🔴 Cancelling deposit: ${body.pendingPaymentId} for jar ${jarId}`);

    await paymentService.cancelSavingsFunding(body.pendingPaymentId, jarId);

    console.log(`✅ Deposit cancelled: ${body.pendingPaymentId} for jar ${jarId}`);
    
    return sendResponse.success(ctx, "Deposit cancelled", 200, { cancelled: true });
  }
}

export const savingsController = new SavingsController();
