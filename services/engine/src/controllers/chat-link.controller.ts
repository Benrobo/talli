...
class ChatLinkController {
  async createCode(ctx: Context) {
    ...
    const issued = await chatLinkService.issueCode(userId, platform, purpose);
  ...
  async listConnectedChats(ctx: Context) {
    ...
    const chats = await chatLinkService.listConnectedChats(userId);
  ...
  async disconnectChat(ctx: Context) {
    ...
    await chatLinkService.disconnectById(userId, linkedChatId);
